...
async function createBlackIcons() {
  const sourceImage = 'backup-original/icon-1024-black.png';
  const outputDir = '.';

  // Windows 桌面快捷方式和任务栏需要的完整尺寸集合
  const sizes = [16, 20, 24, 32, 40, 48, 64, 128, 256, 512];

  console.log('开始处理黑色背景图标...');
  ...
  try {
    ...
    // 更新 icon.png 和 icon-1024.png
    console.log('\n更新主图标文件...');
    await fs.promises.copyFile(sourceImage, 'icon-1024.png');
    await sharp(sourceImage)
      .resize(512, 512, {
        kernel: sharp.kernel.lanczos3,
      })
      ...
      .toFile('icon.png');
    console.log('✓ 已更新 icon.png 和 icon-1024.png');
    ...
    console.log('- icon.png, icon-1024.png');
