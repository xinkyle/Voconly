...
pub fn get_model_path(model_id: &str, backend: &str) -> Result<PathBuf, String> {
    ...
    // GGUF models (TranscribeCpp backend) use file structure
    let is_gguf_model = backend == "transcribe_cpp"
        || backend == "transcribecpp"
        || backend == "transcribe-cpp"
        || model_id.ends_with(".gguf");

    log::debug!("[get_model_path] 判断模型类型: is_gguf_model={}", is_gguf_model);
    ...
    if is_gguf_model {
        ...
        let is_onnx_model = backend == "onnx"
            || model_id.contains("sensevoice")
            || model_id.contains("sense_voice")
            || model_id.contains("parakeet")
            || model_id.contains("moonshine");

        // Historical: Non-GGUF Qwen3-ASR models use directory structure (zip download)
        // Note: Qwen3-ASR GGUF variants use TranscribeCpp backend
        // "python" here identifies directory-type models, not the actual backend
        let is_python_model =
            backend == "python" || (model_id.contains("qwen3-asr") && !model_id.ends_with(".gguf"));

        log::debug!("[get_model_path] ONNX判断: is_onnx={}, is_python={}", is_onnx_model, is_python_model);

        if is_onnx_model || is_python_model {
            // Return directory path for ONNX and directory-type models
            // e.g., sensevoice-small/ or qwen3-asr-0.6/ (directory)
            ...
            // Return file path for GGUF/TranscribeCpp models
            // e.g., qwen3-asr-0.6-q4.gguf
...
/// Historical: For directory-type models (non-GGUF Qwen3-ASR), checks directory with config.json
pub fn model_exists(model_id: &str, backend: &str) -> bool {
    if let Ok(path) = get_model_path(model_id, backend) {
        ...
        // Historical: Non-GGUF Qwen3-ASR uses directory structure
        // "python" here identifies directory-type models, not the actual backend
        let is_python_model =
            backend == "python" || (model_id.contains("qwen3-asr") && !model_id.ends_with(".gguf"));
        ...
        } else if is_python_model {
            // Historical: Directory-type models check directory with config.json
...
pub async fn download_model_with_source(
    ...
    let backend = if sources
        .iter()
        .any(|s| s.url.contains(".onnx") || s.url.contains(".zip"))
        || model_id.contains("sensevoice")
        || model_id.contains("parakeet")
        || model_id.contains("moonshine")
    {
...
/// Automatically detects backend type based on model_id patterns
#[tauri::command]
pub fn check_model_exists_cmd(model_id: String, backend: Option<String>) -> bool {
    let backend_type = backend.unwrap_or_else(|| {
        // 自动检测 GGUF backend (TranscribeCpp)
        // GGUF 文件通常包含量化标识如 -q4, -q5, -q6, -q8, Q4, Q5, Q6, Q8 等
        if model_id.ends_with(".gguf")
            || model_id.contains("-q4")
            || model_id.contains("-q5")
            || model_id.contains("-q6")
            || model_id.contains("-q8")
            || model_id.contains("_q4")
            || model_id.contains("_q5")
            || model_id.contains("_q6")
            || model_id.contains("_q8")
            || model_id.contains("-Q4")
            || model_id.contains("-Q5")
            || model_id.contains("-Q6")
            || model_id.contains("-Q8")
            ...
            || model_id.contains("_Q8")
        {
            return "transcribe_cpp".to_string();
        }
        ...
        // Historical: Non-GGUF Qwen3-ASR directory structure detection
        if model_id.contains("qwen3-asr") && !model_id.ends_with(".gguf") {
            return "python".to_string();
        }

        // 默认 TranscribeCpp backend（GGUF 模型）
        "transcribe_cpp".to_string()
    });
