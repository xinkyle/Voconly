...
/// Signal handler callback for macOS
extern "C" fn sig_handler(sig: i32) {
    let report = generate_macos_signal_report(sig);
    write_crash_report(&report);
...
#[cfg(target_os = "macos")]
/// Generate crash report from macOS signal
fn generate_macos_signal_report(sig: i32) -> String {
    let timestamp = chrono::Utc::now().format("%Y-%m-%d %H:%M:%S");

    let signal_desc = match sig {
