...
export interface AsrModelListProps {
  ...
  /** Current language for recommendation */
  ...
  /** Currently selected model ID (may include quantization suffix) - optional */
  selectedModelId?: string;
  /** Callback when a model is selected - optional */
  onSelect?: (modelId: string) => void;
  /** Callback when modal is closed after selection - optional */
  onClose?: () => void;
  /** Layout mode: single column or grid */
  layout?: 'single' | 'grid';
}

/**
 * ASR Model List Component
 *
 * Renders a list of ASR model cards with support for:
 * - Model download status display
 * - Quantization version selection
 * - Progress indicators for downloads
 * - Language-based model recommendations
 * - Optional selection functionality
 */
export default function AsrModelList({
  models,
  downloadStates,
  onDownload,
  onDownloadCancel,
  ...
  const sortedModels = sortAsrModels(models, currentLanguage);
  ...
  const getSelectedQuant = (model: AsrModelWithStatus): string | undefined => {
    if (!selectedModelId || !isModelSelected(model)) return undefined;
  ...
  // Handle card click (select model) - only when selection is enabled
  const handleCardClick = async (model: AsrModelWithStatus) => {
    if (!onSelect) return;

    const isSelected = isModelSelected(model);

    // If already selected, just close the modal (if in modal mode)
    if (isSelected) {
      onClose?.();
      return;
    }
  ...
  return (
    <div className={containerClass}>
      {sortedModels.map((model) => {
        ...
        // Determine if the card can select model (only when selection is enabled)
        const canSelect = !!onSelect;

        // Determine if the card can expand (has multiple quant variants)
        const canExpand = hasMultipleQuantVariants || isDefaultQuantDownloaded;

        return (
          <div
            key={model.preset.id}
            onClick={() => canSelect && handleCardClick(model)}
            ...
            <div className="relative px-4 py-3 z-10">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex-shrink-0 text-right" onClick={(e) => handleTagClick(e, model)}>
                  {isDownloading ? (
                    ...
                    <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                      ...
                      {onDownloadCancel && (
                        <button
                          ...
                          className="px-1.5 py-0.5 text-[10px] font-medium text-red-600 bg-red-50 hover:bg-red-100 rounded transition-colors"
                        >
                          {t('models.cancel')}
                        </button>
                      )}
                    ...
                    <div className="flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded bg-gray-100">
                      {/* Download status dot */}
                      <span className={`w-1.5 h-1.5 rounded-full ${isDefaultQuantDownloaded ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                    ...
                    <div
                      className="flex items-center gap-1 px-2 py-1 text-xs font-medium rounded bg-gray-100 cursor-pointer hover:bg-gray-200"
                      onClick={(e) => {
                        e.stopPropagation();
                        startDownload(model);
                      }}
              ...
              {isExpanded && hasMultipleQuantVariants && (
                <div className="mt-2 pt-2 border-t border-gray-100" onClick={e => e.stopPropagation()}>
                  <div className="text-[11px] text-gray-500 mb-1.5">{t('models.quantVersions')}:</div>
                  <div className="space-y-1">
                    {quantVariants.map((variant: QuantVariant) => {
                      ...
                      return (
                        <div
                          key={variant.quant}
                          className={`relative flex items-center justify-between px-2 py-1.5 rounded text-xs transition-colors ${
                            isDownloadedVariant
                              ? isSelectedVariant
                          ...
                          onClick={isDownloadedVariant && !isSelectedVariant && onSelect ? () => {
                        ...
                        >
                          ...
                          <div className="relative flex items-center gap-2 z-10">
                            <span className="text-gray-400 text-[11px]">{formatBytes(variant.sizeBytes)}</span>
                            {isDownloadedVariant ? (
                              isSelectedVariant ? (
                                <span className="text-blue-600 font-medium text-[10px]">{t('models.currentUse')}</span>
                              ) : (
                                <span className="text-[10px] text-gray-400">{t('models.downloaded')}</span>
                              )
                            ) : (
                              <button
                                onClick={(e) => {
