...
import { getAsrModelList, type AsrModelWithStatus, parseModelId, QUANT_LABELS } from '../services/config';
...
interface HomePanelV2Props {
  ...
  onModelQuantPrefsChange?: (modelId: string, quant: string) => void;
...
export default function HomePanelV2({
  ...
  models = [],
  llmProfiles = [],
  modelQuantPrefs: _modelQuantPrefs = {},
  downloadStates = {},
  onDownload,
  onDownloadCancel,
  onSave,
  ...
  // 获取第一个启用的场景（用于显示模型信息）
  const activeScene = localScenes.find(s => s.enabled);
  const asrModelId = activeScene?.model?.modelId ? getFullModelId(activeScene.model) : '';
  const asrModelName = asrModelId ? getModelName(asrModelId, models, asrModels) : t('home.noModelSelected');

  // 获取 LLM 配置
  const llmProfile = activeScene ? llmProfiles.find(p => p.sceneId === activeScene.id) : null;
  const llmModelName = llmProfile?.model || t('home.noModelSelected');
    ...
    <div className="min-h-[400px]">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-5">
          {/* ASR */}
          <button
            onClick={() => activeScene && setSelectingSceneId(activeScene.id)}
                ...
                {asrModelId && (
                  <div className="flex items-center gap-1.5">
                    {(() => {
                      ...
                      const size = getModelSize(asrModelId, models, asrModels);
                      if (quant && size) {
                        return (
                          <>
          ...
          {/* LLM */}
          <button
            onClick={() => activeScene && setLlmConfigScene(activeScene)}
            className="group relative bg-gray-100 border border-gray-200 hover:border-gray-300 rounded-xl p-5 text-left transition-all duration-200 hover:shadow-sm"
          >
            {/* 状态指示器 */}
            {llmProfile?.model && (
        ...
        {enabledScenes.length > 0 ? (
          <div className="space-y-3">
            {enabledScenes.map((scene) => {
              ...
              const promptType = sceneLlmProfile?.userPromptType;
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-colors border bg-gray-50 text-gray-600 border-gray-200`}>
                      {!isLlmEnabled ? (
                        ...
                        <>
                          ...
                          {promptType ? getPromptTypeLabel(promptType, t) : 'LLM'}
                        </>
                      )}
                    </span>
                  </div>
      ...
      {selectingSceneId && (
        <AsrModelSelectModal
          models={asrModels}
          selectedModelId={localScenes.find(s => s.id === selectingSceneId)?.model?.modelId || ''}
      ...
      {selectingPromptTypeSceneId && (() => {
        const currentProfile = llmProfiles.find(p => p.sceneId === selectingPromptTypeSceneId);
        const currentPromptType = currentProfile?.userPromptType || 'lightPolish';
        return (
          <div
